...
data_eval$Percentage=round((data_eval$Eigenvalue/eval_sum)*100,1)

#Plots PCA with unknown samples labelled
pdf(args[3])
ggplot(plot_data_filtered, aes(x=PC1, y=PC2, fill = Status, shape = Status, label = Sample)) + geom_point(size=4,alpha = 0.8) + scale_fill_manual(values = c("Domestic" = "black","Wild" = "grey","Unknown" = "red")) + scale_shape_manual(values = c("Domestic" = 21,"Wild" = 22,"Unknown" = 23)) + theme(panel.background = element_blank(), axis.line.x = element_line(color="black", size = 0.5), axis.line.y = element_line(color="black", size = 0.5)) + labs(x = paste0("PC1 (",data_eval$Percentage[1],"%)"),y = paste0("PC2 (",data_eval$Percentage[2],"%)")) + geom_text_repel(aes(label = label), na.rm = TRUE, force = 2, box.padding = 1, max.overlaps = NA, size = 2.5, segment.linetype= "dashed", segment.size = 0.1) + theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), axis.title.x = element_text(size = 13), axis.title.y = element_text(size = 13), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 0.2), axis.line.y = element_line(color="black", size = 0.2))
dev.off()
